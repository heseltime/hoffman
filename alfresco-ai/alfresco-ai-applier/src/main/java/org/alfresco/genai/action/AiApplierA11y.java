 ...
 */
@Component
public class AiApplierA11y implements AiApplierAction {
    ...
    public boolean execute(ResultSetRowEntry entry) {
        ...
        LOG.debug("A11y-processing (...) document {} ({})", entry.getEntry().getName(), uuid);

        if (renditionService.pdfRenditionIsCreated(uuid)) {

            try {

                nodeUpdateService.updateNodeSummary(uuid, genAiClient.getA11y(renditionService.getRenditionContent(uuid)));
                LOG.debug("Document {} has been updated with ... ", entry.getEntry().getName());
                return true;
